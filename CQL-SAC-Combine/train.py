    ...
    with wandb.init(project="CQL", name=config.run_name, config=config):
        ...
        for path in dataset:
            for idx in range(len(path[0])):
                buffer.add(path[0][idx], path[1][idx], path[2][idx], path[3][idx], path[4][idx])
                cnt += 1
        ...
        for i in range(1, config.episodes+1):
            ...
            while True:
                ...
                buffer.add(state, action, reward, next_state, done) # online combine offline, comment this line for full offline
            ...
            if i % config.save_every == 0:
                save(config, save_name="CQL-SAC-discrete", model=agent.actor_local, wandb=wandb, ep=i)
